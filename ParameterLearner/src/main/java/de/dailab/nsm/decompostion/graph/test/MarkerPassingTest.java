...
import de.dailab.nsm.decomposition.graph.spreadingActivation.DoubleSpreadingActivation.DoubleSpreadingActivation;
...
public class MarkerPassingTest extends SemanticDistanceTest {
    ...
    public <T extends DoubleNodeWithMultipleThresholds> DoubleMarkerPassing getDoubleMarkerPassing(Graph resultGraph, List<Map<Concept, List<? extends Marker>>> startActivation, Map<Concept, Double> threshold, Class<T> nodeType) {
        ...
        doubleMarkerPassingAlgo.doInitialMarking(startActivation, doubleMarkerPassingAlgo);
            ...
            for (DoubleNodeWithMultipleThresholds node : activeNodes) {
                for (Concept concept : node.getActivation().keySet()) {
                    ...
                    if (!node.getActivation(concept).isEmpty()) {
                        //TODO: Discuss: Is this as it was intended (had to refactor here)
                        Map<Double, List<Concept>> activation = node.getActivation(concept);
                        double s  =activation.keySet().stream().mapToDouble(d->d).sum();
                        avgActivation += s;
                    }
                }
            }
    ...
    @Override
    public void test() {
        List<SynonymPair> l = (List)testSynonymPairs;
        for (SynonymPair pair : l) {
            try {

...
enum StartActivation{
    HEADWORD,VERBSANDNOUNS, VERBSNOUNSANDADJECTIVES, ALL
}

enum Evaluation{
    SUMME,SUMMEDURCHWORTANZAHL, SUMMENOUNS, SUMMENOUNSANDVERBS, SUMMENOUNSBERBSANDAFJECTIVES
}
...
//        GraphUtil.setMergedGraphCache(new HashMap<>(65));
    ...
    public static void main(String args[]) throws FileNotFoundException, UnsupportedEncodingException, IOException {
      ...
      Map<QuestionClass,List<Question>> questionMap = DataLoader.getQuestionMap();
          ...
          List<Question> questions = DataLoader.ReadExampleDataSet();
          ...
          int numberOfSchemas = questions.size();


          for (Question schema : questions) {
              clean();
              MarkerPassingConfig.setDecompositionDepth(2);
              Decomposition decomposition = new Decomposition();
              decomposition.init();
              ...
              try {
                  ...
                  List<List<Concept>> startActivations = new LinkedList<>();
                  startActivations.add(allVerbsAndNouns);
                  startActivations.add(StartActivator.getHeadVerbAndNouns(tokenized, srlPipeline, stanPipeline, nerMap, answers));
                  startActivations.add(StartActivator.getAllVerbsNounsAndAdjectives(tokenized, srlPipeline, stanPipeline, nerMap, answers));
                  startActivations.add(allVerbsAndNouns);
                  ...
                  for (StartActivation a : startActivationWay) {
                      try {
                          ...
                          if (a.equals(StartActivation.HEADWORD)) {
                          ...
                          } else if (a.equals(StartActivation.VERBSANDNOUNS)) {
                          ...
                          if (a.equals(StartActivation.VERBSNOUNSANDADJECTIVES)) {
                          ...
                          WinogradDoubleMarkerPassing doubleMarkerPassing = new WinogradDoubleMarkerPassing(decGraph, WinogradDoubleNode.class, syntaxGraph, nerGraph, roleGraph, new MarkerPassingConfig());//TODO actually MarkerPassingConfig is a static class
                          ...
                          for (Evaluation e : evaluations) {
                              ...
                              if (e.equals(Evaluation.SUMME)) {

                                  activationMap.get(a).put(Evaluation.SUMME, activationMap.get(a).get(Evaluation.SUMME) + eva.evaluate(eva.summe(doubleMarkerPassing, answers), answers, schema.rightAwnser, excluded));


                              }
                              if (e.equals(Evaluation.SUMMEDURCHWORTANZAHL)) {

                                  activationMap.get(a).put(Evaluation.SUMMEDURCHWORTANZAHL, activationMap.get(a).get(Evaluation.SUMMEDURCHWORTANZAHL) + eva.evaluate(eva.summeDurchAnzahl(doubleMarkerPassing, answers), answers, schema.rightAwnser, excluded));

                              }
                              if (e.equals(Evaluation.SUMMENOUNS)) {

                                  activationMap.get(a).put(Evaluation.SUMMENOUNS, activationMap.get(a).get(Evaluation.SUMMENOUNS) + eva.evaluate(eva.summeDurchAnzahlNouns(doubleMarkerPassing, answers, stanPipeline, srlPipeline), answers, schema.rightAwnser, excluded));

                              }
                              if (e.equals(Evaluation.SUMMENOUNSANDVERBS)) {

                                  activationMap.get(a).put(Evaluation.SUMMENOUNSANDVERBS, activationMap.get(a).get(Evaluation.SUMMENOUNSANDVERBS) + eva.evaluate(eva.summeDurchAnzahlNounsAndVerbs(doubleMarkerPassing, answers, stanPipeline, srlPipeline), answers, schema.rightAwnser, excluded));

                              }
                              if (e.equals(Evaluation.SUMMENOUNSBERBSANDAFJECTIVES)) {

                                  activationMap.get(a).put(Evaluation.SUMMENOUNSBERBSANDAFJECTIVES, activationMap.get(a).get(Evaluation.SUMMENOUNSBERBSANDAFJECTIVES) + eva.evaluate(eva.summeDurchAnzahlNounsAndVerbs(doubleMarkerPassing, answers, stanPipeline, srlPipeline), answers, schema.rightAwnser, excluded));
...
//            }
        ...
        for (Map.Entry<String, Integer> entry : rightLinksMap.entrySet()){
          ...
          for (StartActivation a : startActivationWay) {
              ...
              for (Evaluation e : evaluations) {
                  System.out.println("StartActivation: " + questions.size() + " Evaluation: " + e.name() + " Damit ergibt sich ein Score von: " + Evaluator.getScore(activationMap.get(a).get(e), questions));
                  fw = new FileWriter("Ergebnis" + ".txt", true);
                  fw.write("Anazhl: "+ questions.size() +" StartActivation: " + a.name() + " Evaluation: " + e.name() + " Damit ergibt sich ein Score von: " + Evaluator.getScore(activationMap.get(a).get(e), questions));

...
import de.kimanufaktur.nsm.graph.entities.links.DefinitionOfLink
import de.kimanufaktur.nsm.graph.entities.links.HolonymLink
...
import de.kimanufaktur.nsm.graph.entities.links.NameOfLink
...
import de.kimanufaktur.nsm.graph.entities.links.SemanticRoleOfLink
...
fun Link.reverseByType(edgeType: EdgeType, config: ArcConfig) = when (edgeType) {
    EdgeType.SemanticRole -> SemanticRoleOfLink().also { it.weight = config.semanticRoleOfLinkWeight }.reverse(source, target)
    EdgeType.Definition -> DefinitionOfLink().also { it.weight = config.definitionOfLinkWeight }.reverse(source, target)
    EdgeType.Meronym -> HolonymLink().also { it.weight = config.holonymLinkWeight }.reverse(source, target)
    ...
    EdgeType.NamedEntity -> NameOfLink().also { it.weight = config.nameOfLinkWeight }.reverse(source, target)

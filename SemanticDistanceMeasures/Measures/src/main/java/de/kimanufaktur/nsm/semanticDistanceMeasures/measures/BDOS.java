...
/**
 * Created by faehndrich on 14.08.15. Chen, D., Jianzhuo, Y., Liying, F., & Bin, S. (2009). Measure
 * semantic distance in wordnet based on directed graph search. 2009 International Conference on
 * E-Learning, E-Business, Enterprise Information Systems, and E-Government, EEEE 2009, 57–60.
...
public class BDOS implements SemanticDistanceMeasureInterface {
  ...
  public double compareConcepts(IConcept c1, IConcept c2) {
    ...
    if (c1.equals(c2)) {
      ...
      shortestPath =
          compareSetsRecursivly(
              stepSet1, wordType1, stepSet2, wordType2, numberOfSteps, shortestPath);
    }
    return (1.0 - (shortestPath / maxPath));
  }

  double compareSetsRecursivly(
      Set<Concept> stepSet1,
      WordType wordType1,
      Set<Concept> stepSet2,
      WordType wordType2,
    ...
    while (!found && (numberOfSteps <= maxPath)) {
      ...
      if (nextSet1.isEmpty()) {
        ...
        newShortestPath =
            compareSetsRecursivly(
                tmpSet1, wordType1, nextSet2, wordType2, numberOfSteps, newShortestPath);
      } else {
        found = true;
...
//      tmpset.addAll(
//          Decomposition.getDictionaries().get(0).getMeronyms(c).values();
//      tmpset.addAll(Decomposition.getDictionaries().get(0).getHypernyms(c).values());
//      tmpset.addAll(Decomposition.getDictionaries().get(0).getHyponyms(c).values());
//    }
